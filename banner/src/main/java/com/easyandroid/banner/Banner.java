...
import com.easyandroid.banner.loader.ImageLoaderInterface;
import com.easyandroid.banner.view.BannerViewPager;
...
/**
 * package: com.easyandroid.banner.Banner
 * author: gyc
 * description:banner，轮播框架
 * time: create at 2019/6/13 0013 下午 22:51
 */
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private int bannerDefaultResource;
    private int bannerPaddingLeft;
    private int bannerPaddingRight;
    private int bannerCorner;
    private int layoutBackgroundResource;//整个banner布局背景，可以配合mLayoutResId实现自定义背景
    ...
    public void setSelectedDrawable(GradientDrawable selectedDrawable) {
        this.selectedDrawable = selectedDrawable;
    }

    public void setUnSelectedDrawable(GradientDrawable unSelectedDrawable) {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        bannerDefaultImage.setImageResource(bannerDefaultResource);
        bannerContainer.setBackgroundResource(layoutBackgroundResource);
        initViewPagerScroll();
    }
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        bannerDefaultResource = typedArray.getResourceId(R.styleable.Banner_banner_default_image, R.drawable.no_banner);
        bannerPaddingLeft = typedArray.getDimensionPixelSize(R.styleable.Banner_banner_padding_left, BannerConfig.PADDING_LEFT);
        bannerPaddingRight = typedArray.getDimensionPixelSize(R.styleable.Banner_banner_padding_right, BannerConfig.PADDING_RIGHT);
        bannerCorner = typedArray.getDimensionPixelSize(R.styleable.Banner_banner_corner, BannerConfig.BANNER_CORNER);
        layoutBackgroundResource = typedArray.getResourceId(R.styleable.Banner_banner_background_resource, Color.TRANSPARENT);
        typedArray.recycle();
        paramsSelected = new LinearLayout.LayoutParams(mIndicatorSelectedWidth, mIndicatorHeight);
        paramsUnselected = new LinearLayout.LayoutParams(mIndicatorUnselectedWidth, mIndicatorHeight);
        paramsSelected.leftMargin = mIndicatorMargin;
        paramsSelected.rightMargin = mIndicatorMargin;
        paramsUnselected.leftMargin = mIndicatorMargin;
        paramsUnselected.rightMargin = mIndicatorMargin;
    }
    ...
    public Banner setBannerPaddingLeft(int bannerPaddingLeft) {
        this.bannerPaddingLeft = bannerPaddingLeft;
        return this;
    }

    public Banner setBannerPaddingRight(int bannerPaddingRight) {
        this.bannerPaddingRight = bannerPaddingRight;
        return this;
    }

    public Banner setBannerCorner(int bannerCorner) {
        this.bannerCorner = bannerCorner;
        return this;
    }
    ...
    private void setImageList(List<?> imagesUrl) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            View rootView = null;
            if (imageLoader != null) {
                rootView = imageLoader.createView(context);
            }
            if (rootView == null) {
                rootView = LayoutInflater.from(context).inflate(R.layout.layout_img, null);
...
//                imageView = new ImageView(context);
            ...
            bannerViews.add(rootView);
            if (imageLoader != null)
                imageLoader.displayImage(context, url, rootView);
            else
...
//                imageView.setImageResource(mIndicatorSelectedResId);
                addIndicator(imageView, paramsSelected);
            } else {
...
//                imageView.setImageResource(mIndicatorUnselectedResId);
                addIndicator(imageView, paramsUnselected);
            }
            indicatorImages.add(imageView);
        }
    }

    private void addIndicator(ImageView imageView, LinearLayout.LayoutParams params) {
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
                bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE)
...
//        Log.i(tag, ev.getAction() + "--" + isAutoPlay);
    ...
    class BannerPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            container.addView(bannerViews.get(position));
...
//        Log.i(tag,"currentItem: "+currentItem);
    ...
    public void onPageSelected(int position) {
        ...
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
                bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE ||
                bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE) {
            int pLast = (lastPosition - 1 + count) % count;
            int pNow = (position - 1 + count) % count;
            if (mIndicatorSelectedWidth != mIndicatorUnselectedWidth) {
                indicatorImages.get(pLast).setLayoutParams(paramsUnselected);
                indicatorImages.get(pNow).setLayoutParams(paramsSelected);
            }
            indicatorImages.get(pLast).setImageResource(mIndicatorUnselectedResId);
            indicatorImages.get(pNow).setImageResource(mIndicatorSelectedResId);
